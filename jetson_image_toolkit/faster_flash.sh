...
# 1. Check if the cache exists AND is not an empty ghost folder
if [[ -d "${CACHE_DIR}/images" ]] && [[ -n "$(ls -A "${CACHE_DIR}/images" 2>/dev/null)" ]]; then
    log "✅ Found cached extraction for '${BASENAME}'. Skipping unzip!"
else
    if [[ ! -f "${ZIP_PATH}" ]]; then
    ...
    log "📦 Extracting ${ZIP_PATH} to network-safe cache..."
...
# 2. AGGRESSIVE SUDO CLEANUP (NO MORE HOARDING)
log "🧹 Force-cleaning L4T directories to reclaim space..."
sudo rm -rf "${IMAGES_DIR}"
# Nuke ANY _prev folders instantly
sudo rm -rf "${L4T_DIR}/tools/backup_restore/images_"* 2>/dev/null || true

# 3. CREATE THE SYMLINK
log "🔗 Linking ${BASENAME} into Linux_for_Tegra for flashing..."
cd "${L4T_DIR}/tools/backup_restore"
sudo ln -snf "extracted_caches/${BASENAME}/images" "images"

# 4. PRINT PROOF TO THE GUI LOGS!
log "🔎 SYMLINK VERIFICATION PROOF:"
ls -la images
cd "${REPO_ROOT}"
# --- END CACHING LOGIC ---

log "🚀 Restoring board ${BOARD} using images for ${BASENAME}"
